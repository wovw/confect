...
You can declare an `error` schema on a function spec to model expected domain failures. When `error` is present, Confect returns `Either<Success, DomainError>` on the client.
...
const getById = FunctionImpl.make(
  api,
  "notes",
  "getById",
  ({ noteId }) =>
...
Unhandled defects still throw and appear in your query client's `error` channel. Declared domain errors are encoded in `Either.left(...)` and returned in `data`.
