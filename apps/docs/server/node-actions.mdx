...
A node spec uses `Spec.defineNode()` and `GroupSpec.defineNode()` to define groups, and `FunctionSpec.publicNodeAction()` or `FunctionSpec.internalNodeAction()` to define functions.
...
export default Spec.defineNode({ email });
```
